...
export default {
  ...
  datasource: {
    ...
    source: {
      ...
      "directoryHint": "Empty selects the authorized root, or enter src or docs",
      "rootHint": "This is a server directory, not the browser computer. Operators approve and mount roots read-only.",
      "browse": "Browse read-only sources",
      "files": "files",
      "searchHint": "Search filenames or code text",
      "skipped": "{count} entries skipped by exclusions, binary/encoding detection or size limits.",
      "partial": "Results are incomplete. Narrow the directory or refine the query.",
      "noMatches": "No matches",
      "origin": "Open repository source",
      "localCitation": "No verified remote commit link is available. Cite the current platform snapshot.",
      "partialRead": "Content was truncated; read a smaller line range.",
      "selectFile": "Select a file or search result to read it.",
      "noSources": "No read-only sources. Add and sync one in data source settings.",
      "unavailable": "Source unavailable. Check sync status and permissions.",
      "archiveHint": "Download a pinned read-only snapshot without a language extension whitelist. Limits: 50000 text files, 256 MiB total, 4 MiB per file. Skipped entries are reported."
},
    github: {
  ...
  agentEditor: {
    ...
    tools: {
      ...
      githubReleaseLookup: 'Поиск релизов GitHub',
      githubReleaseLookupDesc: 'Запрашивает официальные релизы, заметки к релизам и теги разрешённых репозиториев GitHub; версия ветки разработки или тег не считаются опубликованным релизом.',
  ...
  input: {
    ...
    steerAfter: 'Отправить после завершения',
    steerAccepted: 'Дополнение к текущей задаче',
    ...
    steerQueueInjecting: 'Ожидание применения дополнения',
  ...
  envVarSettings: {
    ...
    storedPlaceholder: 'Сохранено, введите новое значение для замены',
  ...
  settings: {
    ...
    skills: {
      ...
      addProgress: 'Ход добавления',
      addStepRegister: 'Регистрация',
      addStepInstall: 'Установка',
      addStepRegisterDesc: 'Вставьте источник или загрузите zip, чтобы сначала добавить навык в каталог пространства.',
      addStepInstallDesc: 'Проверьте разобранный навык, затем выберите песочницы. Можно пропустить и установить позже из списка.',
      addFinish: 'Готово',
      ...
      installToSandbox: 'Установить в песочницу',
      installToSandboxDesc: 'Каждый бэкенд песочницы требует отдельной установки. Образы Docker, Cube и E2B несовместимы.',
      pickSandboxes: 'Установить в песочницы',
      pickSandboxesHint: 'Навык записывается в образ выбранной песочницы. Образы Docker, Cube и E2B несовместимы — устанавливайте отдельно в каждую. Можно пропустить и установить позже из каталога.',
  ...
  knowledgeBase: {
    ...
    deleteTaskFailed: 'Не удалось удалить документы. Проверьте сведения об ошибке и повторите попытку.',
