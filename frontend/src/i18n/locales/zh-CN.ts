...
export default {
  ...
  datasource: {
    ...
    source: {
      ...
      "documents": "文档入库",
      "readonly": "源码与文本只读",
      "sourceHint": "保留完整目录，支持各种语言的文本源码。用于搜索、按行读取和引用，不执行代码，也不向量化。",
      "documentHint": "使用知识库现有解析与索引设置。更换用途不会删除已导入的文档；可分别添加文档源和只读源。",
      "excludes": "排除目录或文件（每行一个）",
      "excludesHint": "默认排除依赖与构建目录，可修改。已知密钥文件、版本库内部文件和符号链接不读取。",
      "root": "服务器目录",
      "chooseRoot": "选择已授权的服务器目录",
      "directory": "子目录",
      "directoryHint": "留空读取整个授权目录，例如 src 或 docs",
      "rootHint": "这里选择服务器上的目录，不是浏览器所在电脑的路径。根目录由运维批准并只读挂载。",
      "browse": "浏览只读来源",
      "files": "个文件",
      "searchHint": "搜索文件名或代码文本",
      "skipped": "有 {count} 个条目因排除规则、二进制、编码或大小限制跳过。",
      "partial": "结果未穷尽，请缩小目录或调整关键词。",
      "noMatches": "未找到匹配结果",
      "origin": "打开仓库来源",
      "localCitation": "该文件没有可核验的远程提交链接；以当前平台快照为依据。",
      "partialRead": "内容已截断，可缩小行范围继续读取。",
      "selectFile": "选择目录中的文件或搜索结果查看内容。",
      "noSources": "本知识库尚无可用只读来源，请在数据源设置中添加并同步。",
      "unavailable": "来源暂不可用，请检查同步状态和访问权限。",
      "archiveHint": "下载固定提交的只读快照，不按编程语言扩展名筛选。文本文件最多 50000 个、合计 256 MiB，单文件 4 MiB；其他条目会显示跳过原因。"
},
    github: {
  ...
  settings: {
    ...
    skills: {
      ...
      addStepRegister: '登记',
      addStepInstall: '安装',
      addStepRegisterDesc: '粘贴来源或上传 zip，先登记到空间目录。',
      addStepInstallDesc: '确认解析结果后，选择要写入的沙箱。也可以跳过，稍后在列表里安装。',
      addFinish: '完成',
      ...
      installToSandboxDesc: '每种沙箱后端要单独安装一次。Docker、Cube、E2B 的镜像互不通用。',
      pickSandboxes: '安装到哪些沙箱',
      pickSandboxesHint: '技能会写入所选沙箱当前使用的镜像。Docker、Cube、E2B 互不通用，勾几份就装几次。现在可以不选，稍后在目录里再装。',
