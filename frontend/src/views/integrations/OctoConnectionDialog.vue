<template>
  <t-dialog v-model:visible="visible" header="连接 Octo Bot" :confirm-loading="saving" :confirm-btn="{ content: identity ? '保存已核验的连接' : '核验 Bot 身份', disabled: !token.trim() || !account.trim() }" @confirm="confirm" @closed="reset">
