...
// PreviewDataSourceDelete godoc
// @Summary Preview a data source deletion
// @Description Returns the generated knowledge affected by a purge and a short-lived confirmation token. This endpoint does not change data.
// @Tags DataSource
// @Param id path string true "Data source ID"
// @Success 200 {object} types.DataSourceDeletePreview
// @Failure 404 {object} map[string]string
// @Router /datasource/{id}/delete-preview [post]
func (h *DataSourceHandler) PreviewDataSourceDelete(c *gin.Context) {
	ctx := c.Request.Context()
	tenantID := h.getTenantID(c)
	if tenantID == 0 {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
	...
	preview, err := h.service.PreviewDataSourceDelete(ctx, id)
	if err != nil {
		h.writeDataSourceDeleteError(c, err)
		return
	}
	c.JSON(http.StatusOK, preview)
}

// DeleteDataSourceWithMode godoc
// @Summary Delete a data source with an explicit content mode
// @Description mode=detach keeps imported knowledge. mode=purge_generated requires a fresh preview_token and removes only knowledge generated by this source.
// @Tags DataSource
// @Accept json
// @Produce json
// @Param id path string true "Data source ID"
// @Param request body types.DataSourceDeleteRequest true "Explicit delete mode"
// @Success 200 {object} types.DataSourceDeleteResult
// @Failure 400 {object} map[string]string
// @Failure 409 {object} map[string]string
// @Router /datasource/{id}/delete [post]
func (h *DataSourceHandler) DeleteDataSourceWithMode(c *gin.Context) {
	ctx := c.Request.Context()
	tenantID := h.getTenantID(c)
	if tenantID == 0 {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid data source delete request"})
		return
	}
	result, err := h.service.DeleteDataSourceWithMode(ctx, id, &req)
	if err != nil {
		h.writeDataSourceDeleteError(c, err)
		return
	}
...
func (h *DataSourceHandler) writeDataSourceDeleteError(c *gin.Context, err error) {
