...
func (s *Service) Report(ctx context.Context, f IssueFilter) (*Report, error) {
	...
	r := &Report{Counts: map[string]int64{}, Items: []Issue{}, From: f.From, To: f.To, GeneratedAt: time.Now().UTC(), Scope: p.ScopeName, Notice: "Only registered issues are counted. Closing an issue does not mean knowledge was updated. Time range filters issue creation; end is exclusive."}
	if p.IsDirect {
		r.Scope = "Current private conversation"
	}
	if p.Console {
		r.Scope = "Authorized workspace issues"
	}
	var groups []struct {
		Status string
		Count  int64
...
func ReportFile(r *Report, format string) (*ReportArtifact, error) {
	...
	case "csv":
		...
		_ = writer.Write([]string{"ID", "Type", "Title", "Reporter", "Owner", "Status", "Created", "Source message", "Report scope", "Report period"})
		for _, row := range r.Items {
			_ = writer.Write([]string{safeCSV(row.ID), safeCSV(row.Kind), safeCSV(row.Title), safeCSV(row.ReporterName), safeCSV(row.OwnerName), row.Status, row.CreatedAt.Format(time.RFC3339), safeCSV(row.MessageID), safeCSV(r.Scope), period})
		}
		writer.Flush()
	...
	case "html":
		...
		for _, row := range r.Items {
			fmt.Fprintf(&b, "<tr><td>%s<br><small>%s</small></td><td>%s</td><td>%s</td><td>%s</td></tr>", html.EscapeString(row.Title), html.EscapeString(row.ID), html.EscapeString(row.ReporterName), html.EscapeString(row.OwnerName), html.EscapeString(row.Status))
		}
	...
	if r.Truncated {
		if format == "html" {
			...
			b.WriteString("\nList truncated to newest 500 registered issues; counts include all matching issues.\n")
		}
	}
