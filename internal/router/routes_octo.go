...
func RegisterOctoRoutes(r *gin.RouterGroup, h *octointegration.Handler, g *rbacGuards) {
	...
	admin.POST("/connections/probe", h.ProbeConnection)
	admin.GET("/connections/:account_id/identity", h.ConnectionIdentity)
	admin.GET("/connections/:account_id/available-scopes", h.AvailableScopes)
	...
	admin.GET("/scopes/:scope_id/managed-knowledge-bases", h.ManagedKnowledgeBases)
	admin.DELETE("/scopes/:scope_id/managed-knowledge-bases/:id", g.KBAccessWrite("id"), h.RevokeKnowledgeManagement)
