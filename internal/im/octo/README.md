...
- 文本附件读取成功，知识条目数保持不变。
- 空绑定消息不进入模型，也不产生回复。
- 真人私聊经明确白名单进入独立会话，已核对平台上的回复和原始消息关联。

前端完整测试、类型检查和镜像构建通过。Chrome 隔离实例已验收四步配置、已有设置回显、改名保存及数据库回读，连接和 UID 白名单保留且渠道保持停用。主任务正在发布新的来源模块，本分支没有覆盖其主服务，最终统一部署以主任务发布记录为准。Bot 对 Bot 私聊曾被平台以非好友拒绝，随后使用真人原生消息完成验收，没有冒充 UID。

原生 source_browse 已列入只读能力交集，仍须由 Agent 配置显式允许（或采用原生默认工具集），并通过来源模块自身的完整 KB 授权；工具注册和引用链接由来源工作包提供。本分支不复制文件中继和源码引用实现。
